...
int main(int argc, char *argv[])
{
  ...
  bkap4 = cmatrix(0,3,0,NTAU);
    ...
    ///////////////////////// MODIFICONE ///////////////////////////////
  FILE *file1 = fopen("parameters", "r"); // Apertura del file in modalità lettura
      if (file1 == NULL) {
  ...
  float vu;
  float SS, EE, STEP;
  fscanf(file1, "ATMname %s\n", ATMname);
  fscanf(file1, "LINESname %s\n", LINESname);
  fscanf(file1, "OUTname %s\n", OUTname);
  fscanf(file1, "vturb %f\n", &vu);
  fscanf(file1, "startwl %f\n", &SS);
  fscanf(file1, "endwl %f\n", &EE);
  fscanf(file1, "step %f\n", &STEP);
  printf("\n%s\n", ATMname);
  printf("\n%s\n", LINESname);
  printf("\n%s\n", OUTname);
  ...
  ggets(lines);
  if(strcmp(lines,"") == 0) strcpy(lines,LINESname);
  if((qf = fopen(LINESname,"r")) == NULL) {
